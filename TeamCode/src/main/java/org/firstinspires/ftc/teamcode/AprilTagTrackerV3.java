...
public class AprilTagTrackerV3 extends OpMode {
        ...
        switch (state) {
            ...
            case BUILD_PATH:
                follower.setStartingPose(
                        new Pose(fieldX, fieldY, heading));
    ...
    */
    ...
    private void calculateRobotPose() {
        ...
        // Correct camera yaw for upside-down mounting
        double cameraYaw = Math.toRadians(-desiredTag.ftcPose.yaw);

        // Robot heading on the field
        heading = Math.toRadians(45.0) - cameraYaw;

        // Robot position relative to the tag
        double relX = desiredTag.ftcPose.x;
        double relY = desiredTag.ftcPose.y;

        // Known field position of AprilTag 24
        double tagFieldX = 129.1227;
        double tagFieldY = 126.3925;

        // Tag orientation on the field
        double tagAngle = Math.toRadians(225.0);

        // Rotate robot-relative coordinates into field coordinates
        double rotatedX =
                relY * Math.cos(tagAngle)
                        - relX * Math.sin(tagAngle);

        double rotatedY =
                relY * Math.sin(tagAngle)
                        + relX * Math.cos(tagAngle);

        // Final robot field position
        fieldX = tagFieldX + rotatedX;
        fieldY = tagFieldY + rotatedY;
    }
}
