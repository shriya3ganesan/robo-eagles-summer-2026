...
@TeleOp
@Config
public class turretpidtest_pinpoint_limelight extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested()){
            ...
            double desiredAngle = 0 - robotGoalAngle + toDegrees(pinpointpos.heading.toDouble());
            if (desiredAngle < -165) {
                desiredAngle += 360;
            }
            ...
            if (llResult != null && llResult.isValid()){
                xOffset = llResult.getTx();
            }
            else {
                xOffset = 0.0;
                telemetry.addData("target", "INVALID");
            }

            if (currentPos > desiredAngle - 10 && currentPos < desiredAngle + 10){
                double kVision = Range.clip(
                        map(distance,
                                60, 140,
                                0.0, 1.0),
                        0.0, 1.0
                );
                telemetry.addData("kVision", kVision);
                if (llResult != null && llResult.isValid()){
                    telemetry.addData("x offset", xOffset);
                    double angleError = xOffset * kVision;
                    telemetry.addData("error", angleError);
                }
            }
            else {
                telemetry.addData("turret", "NOT IN RANGE");
            }
